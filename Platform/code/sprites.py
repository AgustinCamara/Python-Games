...
class Worm(Enemy):
    def __init__(self, rect, groups, frames):
    ...
    def move(self, dt):
        self.rect.x = self.rect.x + self.speed * dt if self.direction == 1 else self.rect.x - self.speed * dt
...
class Bee(Enemy):
    def __init__(self, pos, groups,frames, speed):
        super().__init__(frames, pos, groups)
        self.speed = speed
        self.amplitude = randint(300, 500)
        self.frecuency = randint(300, 600)

    def move(self, dt):
        self.rect.x -= self.speed * dt
        self.rect.y += sin(pygame.time.get_ticks() / self.frecuency) * self.amplitude * dt
